 ...
 */
var website_RequestURL = "http://168.160.95.141:8040/service/esService.svc?singleWsdl";
//var website_RequestURL = "http://localhost:9138/service/esService.svc?singlewsdl";
// published objects
...
SOAPProxyFabric.fromXml = function (text) {
	alert(SOAPProxyUtils._parseXML(text));
...
SOAPProxyOperation.prototype._buildInput = function (p) {
    var hdr = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" +
              ...
              ">" +
                  "<soap:Body>";
    var ftr =     "</soap:Body></soap:Envelope>";
    var cnt = "";
    ...
    if (cnt == "")
        cnt = "<" + msgname + " xmlns=\"http://tempuri.org/\"/>";
    else {
        cnt = "<" + msgname + " xmlns=\"http://tempuri.org/\">" + cnt + "</" + msgname + ">";
    }
    ...
    for (var i =0; i < attrs.length; i++) {
        ...
        if (an.indexOf(":") != -1) {
            ...
            if (/\/XMLSchema$/.test(attrs[i].nodeValue)) // This is xsd namespace
                this._nsxsd = "xs:";//an.substring(an.indexOf(":")+1) + ":";
        ...
        for (var i = 0; i < ol.length; i++)
            if (ol[i].nodeName == this._nswsdl + "operation") {
                ...
                if (msgin[0].attributes.getNamedItem("message") == null)
                    continue;
                msginname = msgin[0].attributes.getNamedItem("message").nodeValue;
                ...
                if (msgout[0].attributes.getNamedItem("message") == null)
                    continue;
                msgoutname = msgout[0].attributes.getNamedItem("message").nodeValue;
    ...
    if (SOAPProxyUtils._standardTypes == null) {
        ...
        var _serStr = function (o, name, proxy) {
                          if (o == null)
                              ...
                              return "<"+name+">" + SOAPProxyUtils._stringToString(o) + "</"+name+">";
                      };
        ...
        var _serInt = function (o, name, proxy) {
                          if (o == null)
                              ...
                              return "<"+name+">" + SOAPProxyUtils._integerToString(o) + "</"+name+">";
                      };

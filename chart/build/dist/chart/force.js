...
define("echarts/chart/force", ["require", "./base", "../data/Graph", "../layout/Force", "zrender/shape/Line", "zrender/shape/BezierCurve", "zrender/shape/Image", "../util/shape/Icon", "../config", "../util/ecData", "zrender/tool/util", "zrender/config", "zrender/tool/vector", "../chart"], function (e) {
    ...
    function t(e, t, o, h, d) {
        ...
        this.onclick = function (h,i,k) {
            var a;
            if(undefined!=h.target && null!=h.target 
                && undefined!=h.target._echartsData && null!=h.target._echartsData){
                    a=h.target._echartsData._data.url;
                }
    ...
    function i(e) {
        if (this.isDragstart && e.target) {
            ...
            t.fixed = !0,
            this.isDragstart = !1,
            this.zr.on(V.EVENT.MOUSEMOVE, this.onmousemove)
        }
    }
    function n() {
        this._layout.temperature = .8,
        this._step()
    }
    function a(e, t) {
        if (this.isDragend && e.target) {
            ...
            t.needRefresh = !1,
            this.isDragend = !1,
            this.zr.un(V.EVENT.MOUSEMOVE, this.onmousemove)
        }
    }
    function o(e, t, i) {
        var n = y.create();
        return n[0] = (Math.random() - .5) * i + e,
        n[1] = (Math.random() - .5) * i + t,
        n
    }
    var r = e("./base"),
    s = e("../data/Graph"),
    ...
    return t.prototype = {
        ...
        _init : function () {
            ...
            for (var n = 0, a = i.length; a > n; n++) {
                ...
                if (o.type === m.CHART_TYPE_FORCE) {
                    if (i[n] = this.reformOption(i[n]), e = i[n].name || "", this.selectedMap[e] = t ? t.isSelected(e) : !0, !this.selectedMap[e])
                        continue;
                    this.buildMark(n),
                    this._initSerie(o, n);
                    break
        ...
        _getEdgeQueryTarget : function (e, t, i) {
            return i = i || "normal",
            [t.itemStyle && t.itemStyle[i], e.itemStyle[i].linkStyle]
        },
        _initSerie : function (e, t) {
            this._temperature = 1,
            this._graph = e.data ? this._getSerieGraphFromDataMatrix(e) : this._getSerieGraphFromNodeLinks(e),
        ...
        _getSerieGraphFromDataMatrix : function (e) {
            for (var t = [], i = 0, n = [], a = 0; a < e.matrix.length; a++)
                n[a] = e.matrix[a].slice();
            for (var o = e.data || e.nodes, a = 0; a < o.length; a++) {
                var r = {},
                l = o[a];
                for (var h in l)
                    "name" === h ? r.id = l.name : r[h] = l[h];
        ...
        _initLayout : function (e) {
            ...
            this._layout.width = this.parsePercent(e.size, this.zr.getWidth()),
            this._layout.height = this.parsePercent(e.size, this.zr.getHeight()),
            ...
            for (var r = 1 / 0, s =  - (1 / 0), l = 0; i > l; l++) {
                var h = t.nodes[l];
                s = Math.max(h.layout.size, s),
                r = Math.min(h.layout.size, r)
            }
            for (var d = s - r, l = 0; i > l; l++) {
                var h = t.nodes[l];
                d > 0 ? (h.layout.size = (h.layout.size - r) * (a - n) / d + n, h.layout.mass = h.layout.size / a) : (h.layout.size = (a - n) / 2, h.layout.mass = .5)
            }
            for (var l = 0; i > l; l++) {
                var h = t.nodes[l];
                if ("undefined" != typeof this.__nodePositionMap[h.id])
                    h.layout.position = y.create(), y.copy(h.layout.position, this.__nodePositionMap[h.id]);
                else if ("undefined" != typeof h.data.initial)
                    h.layout.position = y.create(), y.copy(h.layout.position, h.data.initial);
                else {
                    var c = this._layout.center,
                    p = Math.min(this._layout.width, this._layout.height);
                    h.layout.position = o(c[0], c[1], .8 * p)
                }
                var m = h.shape.style,
                u = h.layout.size;
                m.width = m.width || 2 * u,
                m.height = m.height || 2 * u,
                m.x = -m.width / 2,
                m.y = -m.height / 2,
            ...
            for (var l = 0; i > l; l++) {
                var g = t.edges[l];
                g.layout.weight > s && (s = g.layout.weight)
            }
            for (var l = 0; i > l; l++) {
                var g = t.edges[l];
                g.layout.weight /= s
        ...
        _buildNodeShapes : function (e, t) {
            ...
            i.eachNode(function (i) {
                ...
                l.style.color || (l.style.color = this.getColor(a ? a.name : i.id)),
                l.style.iconType = this.deepQuery(o, "symbol"),
                l.style.width = l.style.height = 2 * (this.deepQuery(o, "symbolSize") || 0),
                l.style.iconType.match("image") && (l.style.image = l.style.iconType.replace(new RegExp("^image:\\/\\/"), ""), l = new c({
                            style : l.style,
                ...
                this.deepQuery(o, "itemStyle.normal.label.show") && (l.style.text = null == i.data.label ? i.id : i.data.label, l.style.textPosition = this.deepQuery(o, "itemStyle.normal.label.position"), l.style.textColor = this.deepQuery(o, "itemStyle.normal.label.textStyle.color"), l.style.textFont = this.getFont(this.deepQuery(o, "itemStyle.normal.label.textStyle") || {})),
                this.deepQuery(o, "itemStyle.emphasis.label.show") && (l.highlightStyle.textPosition = this.deepQuery(o, "itemStyle.emphasis.label.position"), l.highlightStyle.textColor = this.deepQuery(o, "itemStyle.emphasis.label.textStyle.color"), l.highlightStyle.textFont = this.getFont(this.deepQuery(o, "itemStyle.emphasis.label.textStyle") || {})),
                this.deepQuery(o, "draggable") && (this.setCalculable(l), l.dragEnableTime = 0, l.draggable = !0, l.ondragstart = this.shapeHandler.ondragstart, l.ondragover = null);
                var h = "";
                if ("undefined" != typeof i.category) {
                    var a = n[i.category];
                    h = a && a.name || ""
                }
                u.pack(l, e, t, i.data, i.rawIndex, i.data.name || "", i.category),
        ...
        _buildLinkShapes : function (e, t) {
            for (var i = this._graph, n = i.edges.length, a = 0; n > a; a++) {
                ...
                V = this.deepQuery(m, "type");
                e.linkSymbol && "none" !== e.linkSymbol && (V = "line");
                var y = "line" === V ? h : d,
                U = new y({
                        style : {
                            xStart : 0,
                ...
                if (c && c.shape && (U.style.offset = 4, c.shape.style.offset = 4), g.merge(U.style, this.query(e, "itemStyle.normal.linkStyle"), !0), g.merge(U.highlightStyle, this.query(e, "itemStyle.emphasis.linkStyle"), !0), "undefined" != typeof r.itemStyle && (r.itemStyle.normal && g.merge(U.style, r.itemStyle.normal, !0), r.itemStyle.emphasis && g.merge(U.highlightStyle, r.itemStyle.emphasis, !0)), U.style.lineWidth = U.style.lineWidth || U.style.width, U.style.strokeColor = U.style.strokeColor || U.style.color, U.highlightStyle.lineWidth = U.highlightStyle.lineWidth || U.highlightStyle.width, U.highlightStyle.strokeColor = U.highlightStyle.strokeColor || U.highlightStyle.color, u.pack(U, e, t, o.data, null == o.rawIndex ? a : o.rawIndex, o.data.name || s.id + " - " + l.id, s.id, l.id), this.shapeList.push(U), this.zr.addShape(U), o.shape = U, e.linkSymbol && "none" !== e.linkSymbol) {
                    var f = new p({
                            style : {
                                x : -5,
                                y : 0,
        ...
        _updateLinkShapes : function () {
            for (var e = y.create(), t = y.create(), i = y.create(), n = y.create(), a = this._graph.edges, o = 0, r = a.length; r > o; o++) {
                ...
                y.copy(n, h.position);
                var d = s.shape.style;
                if (y.sub(e, i, n), y.normalize(e, e), d.offset ? (t[0] = e[1], t[1] = -e[0], y.scaleAndAdd(i, i, t, d.offset), y.scaleAndAdd(n, n, t, d.offset)) : "bezier-curve" === s.shape.type && (d.cpX1 = (i[0] + n[0]) / 2 - (n[1] - i[1]) / 4, d.cpY1 = (i[1] + n[1]) / 2 - (i[0] - n[0]) / 4), d.xStart = i[0], d.yStart = i[1], d.xEnd = n[0], d.yEnd = n[1], s.shape.modSelf(), s.shape._symbolShape) {
                    var c = s.shape._symbolShape;
                    y.copy(c.position, n),
                    y.scaleAndAdd(c.position, c.position, e, h.style.width / 2 + 2);
                    var p = Math.atan2(e[1], e[0]);
                    c.rotation = Math.PI / 2 - p,
                    c.modSelf()
                }
            }
        },
        _syncNodePositions : function () {
            for (var e = this._graph, t = 0; t < e.nodes.length; t++) {
                ...
                r === !0 ? r = 1 : isNaN(r) && (r = 0),
                s === !0 ? s = 1 : isNaN(s) && (s = 0),
                o.position[0] += (n[0] - o.position[0]) * (1 - r),
                o.position[1] += (n[1] - o.position[1]) * (1 - s),
                y.copy(n, o.position);
                var l = a.name;
                if (l) {
        ...
        _step : function () {
            ...
            this._layout.temperature > .01 ? this._layout.step(this._steps) : this.messageCenter.dispatch(m.EVENT.FORCE_LAYOUT_END, {}, {}, this.myChart)
        },
        refresh : function (e) {
            if (e && (this.option = e, this.series = this.option.series), this.legend = this.component.legend, this.legend)
    ...
    i.prototype.addEdge = function (e, t, n) {
        if ("string" == typeof e && (e = this._nodesMap[e]), "string" == typeof t && (t = this._nodesMap[t]), e && t) {
            var a = e.id + "-" + t.id;
            if (this._edgesMap[a])
                return this._edgesMap[a];
            var o = new i.Edge(e, t, n);
            return this._directed && (e.outEdges.push(o), t.inEdges.push(o)),
            e.edges.push(o),
            e !== t && t.edges.push(o),
            this.edges.push(o),
            this._edgesMap[a] = o,
    ...
    i.prototype.removeEdge = function (e) {
        ...
        n = e.node2,
        a = i.id + "-" + n.id;
        this._directed && (i.outEdges.splice(t.indexOf(i.outEdges, e), 1), n.inEdges.splice(t.indexOf(n.inEdges, e), 1)),
        i.edges.splice(t.indexOf(i.edges, e), 1),
        i !== n && n.edges.splice(t.indexOf(n.edges, e), 1),
    ...
    i.prototype.getEdge = function (e, t) {
        ...
        this._directed ? this._edgesMap[e + "-" + t] : this._edgesMap[e + "-" + t] || this._edgesMap[t + "-" + e]
    },
    ...
    i.prototype.eachNode = function (e, t) {
        for (var i = this.nodes.length, n = 0; i > n; n++)
            this.nodes[n] && e.call(t, this.nodes[n], n)
    },
    i.prototype.eachEdge = function (e, t) {
        for (var i = this.edges.length, n = 0; i > n; n++)
            this.edges[n] && e.call(t, this.edges[n], n)
    },
    ...
    return i.Node = n,
    i.Edge = a,
    i.fromMatrix = function (e, t, n) {
        if (t && t.length && t[0].length === t.length && e.length === t.length) {
            for (var a = t.length, o = new i(n), r = 0; a > r; r++) {
                var s = o.addNode(e[r].id, e[r]);
                s.data.value = 0,
                n && (s.data.outValue = s.data.inValue = 0)
            }
            for (var r = 0; a > r; r++)
                for (var l = 0; a > l; l++) {
                    var h = t[r][l];
                    n && (o.nodes[r].data.outValue += h, o.nodes[l].data.inValue += h),
                    o.nodes[r].data.value += h,
    ...
    s.prototype.init = function (e, t) {
        if (this._layoutWorker && (this._layoutWorker.terminate(), this._layoutWorker = null), i && t)
            try {
                this._layoutWorker || (this._layoutWorker = new Worker(i), this._layoutWorker.onmessage = this._$onupdate),
                this._layout = null
            } catch (a) {
                this._layoutWorker = null,
                this._layout || (this._layout = new n)
            }
        else
            this._layout || (this._layout = new n);
        this.temperature = 1,
        ...
        for (var o = e.nodes.length, s = new r(2 * o), l = new r(o), h = new r(o), d = 0; o > d; d++) {
            var c = e.nodes[d];
            s[2 * d] = c.layout.position[0],
            s[2 * d + 1] = c.layout.position[1],
            l[d] = "undefined" == typeof c.layout.mass ? 1 : c.layout.mass,
            h[d] = "undefined" == typeof c.layout.size ? 1 : c.layout.size,
            c.layout.__index = d
        }
        o = e.edges.length;
        for (var p = new r(2 * o), m = new r(o), d = 0; o > d; d++) {
            var u = e.edges[d];
            p[2 * d] = u.node1.layout.__index,
            p[2 * d + 1] = u.node2.layout.__index,
            m[d] = u.layout.weight || 1
        }
        this._layoutWorker ? this._layoutWorker.postMessage({
            ...
            edges : p,
            edgesWeight : m
        }) : (this._layout.initNodes(s, l, h), this._layout.initEdges(p, m)),
        this.updateConfig()
    },
    s.prototype.step = function (e) {
        ...
        if (this._layoutWorker) {
            for (var i = new r(2 * t.length), n = 0; n < t.length; n++) {
                var s = t[n];
                i[2 * n] = s.layout.position[0],
                i[2 * n + 1] = s.layout.position[1]
            }
    ...
    s.prototype._$onupdate = function (e) {
        if (this._layoutWorker) {
            for (var t = new Float32Array(e.data), i = 0; i < this.graph.nodes.length; i++) {
                ...
                n.layout.position[0] = t[2 * i],
                n.layout.position[1] = t[2 * i + 1]
            }
    ...
    function n() {
        this.position = r.create(),
        this.force = r.create(),
        this.forcePrev = r.create(),
        this.speed = r.create(),
        this.speedPrev = r.create(),
        this.mass = 1,
        this.inDegree = 0,
        this.outDegree = 0
    }
    ...
    function o() {
        ...
        this._rootRegion.centerOfMass = r.create(),
        this._massArr = null,
        this._k = 0
    }
    var r,
    s = "undefined" == typeof window && "undefined" == typeof t;
    r = s ? {
        create : function (e, t) {
        ...
        normalize : function (e, t) {
            ...
            n = t[1],
            a = i * i + n * n;
            return a > 0 && (a = 1 / Math.sqrt(a), e[0] = t[0] * a, e[1] = t[1] * a),
            e
        },
        negate : function (e, t) {
            return e[0] = -t[0],
        ...
        for (var e = 0; e < this.nSubRegions; e++)
            ...
            if (this.updateBBox(), this._k = .4 * this.scaling * Math.sqrt(this.width * this.height / e), this.barnesHutOptimize) {
                ...
                for (var t = 0; e > t; t++) {
                    var a = this.nodes[t];
                    i += a.mass,
                    r.scaleAndAdd(n, n, a.position, a.mass)
                }
                i > 0 && r.scale(n, n, 1 / i)
            }
            this.updateForce(),
            ...
            for (var e = this.nodes.length, t = 0; e > t; t++) {
                var i = this.nodes[t];
                r.copy(i.forcePrev, i.force),
                r.copy(i.speedPrev, i.speed),
                r.set(i.force, 0, 0)
            }
            ...
            for (var e = this.nodes.length, t = r.create(), i = 0; e > i; i++) {
                ...
                s = Math.min(o, 500) / o;
                r.scale(n.force, n.force, s),
                r.add(a, a, n.force),
                r.scale(a, a, this.temperature),
                r.sub(t, a, n.speedPrev);
                var l = r.len(t);
                if (l > 0) {
                    r.scale(t, t, 1 / l);
            ...
            return function (t, i) {
                if (t.node)
                    this.applyNodeToNodeRepulsion(t.node, i, !0);
                else {
                    if (0 === t.mass && 0 === i.mass)
                        return;
                    ...
                    if (n > this.barnesHutTheta * t.size * t.size) {
                        ...
                        for (var o = 0; o < t.nSubRegions; o++)
                            this.applyRegionToNodeRepulsion(t.subRegions[o], i)
                }
            }
        }
            ...
            return function (t, i, n) {
                if (t !== i && (0 !== t.mass || 0 !== i.mass)) {
                    ...
                    if (0 !== a) {
                        ...
                        l = Math.sqrt(a);
                        r.scale(e, e, 1 / l),
                        this.preventNodeOverlap ? (l = l - t.size - i.size, l > 0 ? o = this.nodeToNodeRepulsionFactor(s, l, this._k) : 0 >= l && (o = this._k * this._k * 10 * s)) : o = this.nodeToNodeRepulsionFactor(s, l, this._k),
                        n || r.scaleAndAdd(t.force, t.force, e, 2 * o),
                        r.scaleAndAdd(i.force, i.force, e, 2 * -o)
                    }
                }
            }
        }
            ...
            return function (t) {
                ...
                if (!(this.preventOverlap && (o = o - i.size - n.size, 0 >= o))) {
                    ...
                    r.scaleAndAdd(i.force, i.force, e, -s),
                    r.scaleAndAdd(n.force, n.force, e, s)
                }
            }
        }
            ...
            var e = r.create();
            return function (t) {
                r.sub(e, this.center, t.position),
            ...
            return function (n, a) {
                ...
                s = n.node2;
                if (o !== a && s !== a) {
                    r.sub(e, s.position, o.position),
                    r.sub(t, a.position, o.position);
                    var l = r.len(e);
                    r.scale(e, e, 1 / l);
                    var h = r.dot(e, t);
                    if (!(0 > h || h > l)) {
                        ...
                        r.sub(e, a.position, i),
                        r.normalize(e, e),
                        r.scaleAndAdd(a.force, a.force, e, c),
                        r.scaleAndAdd(o.force, o.force, e, -c),
                        r.scaleAndAdd(s.force, s.force, e, -c)
                    }
                }
            }
        }
            ...
            self.onmessage = function (e) {
                if (e.data instanceof ArrayBuffer) {
                    ...
                    for (var t = new Float32Array(e.data), i = t.length / 2, n = 0; i > n; n++) {
                        var a = h.nodes[n];
                        a.position[0] = t[2 * n],
                        a.position[1] = t[2 * n + 1]
                    }
                } else
                    switch (e.data.cmd) {
                    case "init":
                        h || (h = new o),
                        h.initNodes(e.data.nodesPosition, e.data.nodesMass, e.data.nodesSize),
                        h.initEdges(e.data.edges, e.data.edgesWeight);
                        break;
                    case "updateConfig":
                        if (h)
                            for (var r in e.data.config)
                                h[r] = e.data.config[r];
                        break;
                    case "update":
                        var s = e.data.steps;
